...
File1 = "variableMetadata" = output peak calling 
	...
	- in case of use Univariate analysis: add "MetaboliteName" column (columnnr set to 21 instead of 20) with names of standards, CompIDs, or other unique identifiers 

"M3" = matrix3, alternative input to VM for ML with REIMS data. M3 is made automatically in part 1: pre-processing AIMS.
file structure:
	- MZ column fixed
	- intenisty column per burn per sample combined for all samples. Burn selection, no peak picking, no noise filtering are performed.
...
"configalternative(reserve) file" = all settings for the code to run.
- copy configalternative(reserve).py into your project folder in the input folder
- rename to configalternative.py
- Open Configuration.py in notepad
...
- Save configalternative.py
- close configalternative.py
